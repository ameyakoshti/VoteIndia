...
public class VoteIndia extends HttpServlet {
		...
		for (year = 2012; year >= 2000; year--) {
			...
			for (int cmIndex = 0; cmIndex < listOfCMs.size(); cmIndex++) {
				if (Integer.parseInt(listOfCMs.get(cmIndex).startYear) <= year && year <= Integer.parseInt(listOfCMs.get(cmIndex).endYear)) {
					XML += "<cm cmname=\"" + listOfCMs.get(cmIndex).cmName + "\" img=\""+listOfCMs.get(cmIndex).imageURL + "\"/>";
					XML += "<cm cmname=\"" + listOfCMs.get(cmIndex).cmName + "\" img=\""+listOfCMs.get(cmIndex).imageURL + "\"/>";
				}
			}
			XML += "</cms>";
	...
	public static void generateYearWiseScore() {
		...
		for (int year = 2000; year < 2013; year++) {
			...
			Double normalizedScore = 5 + (yearScore / (availableFactors))/20;
			yearBasedScore.put(year, round(normalizedScore,2));
		}
	}
		...
		try {
			...
			try {
				String queryStartDate = "PREFIX ex:<http://example.com/>" + "PREFIX cm:<http://example.com/CM/>" + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>"
						+ "PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>" + "PREFIX foaf: <http://xmlns.com/foaf/0.1/>" + "SELECT ?cmname ?startyear ?img where {" + "?cm rdf:type ex:CM."
						+ "?cm ex:state \"" + state + "\"." + "?cm foaf:name ?cmname." + "?cm cm:joinOffice ?startyear. ?cm cm:thumbnail ?img.}";
				...
				String queryEndDate = "PREFIX ex:<http://example.com/>" + "PREFIX cm:<http://example.com/CM/>" + "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>"
						+ "PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>" + "PREFIX foaf: <http://xmlns.com/foaf/0.1/>" + "SELECT ?cmname ?endyear ?img where {" + "?cm rdf:type ex:CM."
						+ "?cm ex:state \"" + state + "\"." + "?cm foaf:name ?cmname." + "?cm cm:endOffice ?endyear. ?cm cm:thumbnail ?img. }";
				...
				try {
					...
					while (resultStartDate.hasNext()) {
						...
						cmObj.imageURL = bsStartDate.getValue(bindingNamesStartDate.get(2)).toString().replace("\"", "");
